        ...
        if(len > 0)
        {
            ...
            for(int i = 0; i < (int)member_names.size(); ++i)
            {
                ...
                for(unsigned int index = 0; index < (int)parameter.size(); ++index)
                {
                    ...
                    {
                        // Skip CalibrationStatus entries (e.g. L_CalibrationStatus, R_CalibrationStatus)
                        // Valid joint names are l0..l27 / r0..r27: digit at position 1
                        if (Name.size() < 2 || !isdigit((unsigned char)Name[1])) continue;
                        Parameter item;
                        item.name = Name;
                        try {
                            item.value = (float)stod(parameter[index]["Value"].asString());
...
vector<Vector3Float> UDEGloveSDK::GetVecFingerData(string RoleName)
{
    ...
    for(auto &gloves: gloveDataList)
    {
        if(gloves.roleName == RoleName)
        {
            if(gloves.handDatas.fingerJoints.fingerJoint_L.size() < 23) {
                cerr << "Error: Insufficient left hand joint data: "
                     << gloves.handDatas.fingerJoints.fingerJoint_L.size() << endl;
                return GloveVecRes;
            }
            ...
            if(gloves.handDatas.fingerJoints.fingerJoint_R.size() < 23) {
                cerr << "Error: Insufficient right hand joint data: "
                     << gloves.handDatas.fingerJoints.fingerJoint_R.size() << endl;
                return GloveVecRes;
            }
